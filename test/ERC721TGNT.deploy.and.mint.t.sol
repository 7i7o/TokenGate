...
contract ERC72TGNTTest is Test, IERC721TokenReceiver {
    ...
    ERC721TGNTTester private nftTesterTGNT;
    ...
    function testConstructorTGNT() public {
        nftTesterTGNT = new ERC721TGNTTester();
        assertTrue(keccak256(abi.encodePacked(nftTesterTGNT.name())) == keccak256(abi.encodePacked(CONTRACT_NAME)));
        assertTrue(keccak256(abi.encodePacked(nftTesterTGNT.symbol())) == keccak256(abi.encodePacked(CONTRACT_SYMBOL)));
        assertTrue(nftTesterTGNT.balanceOf(address(this)) == 0);
    }

    /** @dev Test minting an NFT to self */
    function testSafeMint() public {
        nftTesterTGNT = new ERC721TGNTTester();
        nftTesterTGNT.safeMint();
        assertTrue(nftTesterTGNT.balanceOf(address(this)) == 1);
        assertTrue(nftTesterTGNT.ownerOf(uint256(uint160(address(this)))) == address(this));
    }
